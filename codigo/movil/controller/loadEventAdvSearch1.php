...
if(isset($_POST['radio']) && isset($_POST['distance']) && isset($_POST['lat']) && isset($_POST['lng']) 
    ...
    //Filter by sector.
    $resultsSector = $dataBase->getEventByCategory($arrayCategories, $con);
     
    $cj = false;
    foreach($arrayCategories AS $sec){
        if($sec==1){
            $cj = true;
        }
    }
    if($cj){
        if($arrayCategories!=""){
            ...
            $result = $dataBase->getEventByCategory($arrayCategories, $con);
        ...
        foreach($results AS $s){
            foreach($resultsSector AS $f){
                if($s->getidEvento() == $f->getidEvento()){
                    $into = true;
                }
            }
            if($into==false){
